...
class ClientController extends Controller
{
    ...
    public function summarizeVisits(Request $request, $id) {
        ...
        $prompt = "あなたは家庭訪問の記録を厳密に整理するアシスタントです。
            以下の【過去の訪問履歴の詳細】に記載されている「明確な事実のみ」を使って、この方の状況がパッと一目でわかるように、100文字〜150文字程度で簡潔な箇条書き(3行以内)で要約してください。

            【絶対厳守の禁止事項】
            ・記載されていない内容からの推測、予測、アドバイスは絶対に含めないでください。
            ・「〜です」「〜ます」などの冗長な表現は避け、事実のみを簡潔に記述してください（不自然な体言止めは不要です）。
            ・箇条書きの各行の頭には、内容がひと目でわかる【見出し】をAIが自動で付けてください。
